...
public class ChuangYeBanController extends Controller {
  public void getStockIds(){
    ...
    System.out.println("ChuangYeBanController.getStockIds(): start = " + start);
    System.out.println("ChuangYeBanController.getStockIds(): limit = " + limit);
    List<Record> list = Db.find("select stockId from stock limit ?,?", start, limit);
    JSONArray jsonArray = new JSONArray();
    System.out.println("ChuangYeBanController.getStockIds(): list.size() = " + list.size());
    for ( int i = 0 ; i < list.size() ; i++ ) {
      Record record = list.get(i);
      jsonArray.put(record.getStr("stockId"));
    }
    System.out.println("ChuangYeBanController.getStockIds(): jsonArray = " + jsonArray);
